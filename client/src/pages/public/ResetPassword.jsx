...
export default function ResetPassword() {
  ...
  const hasNumOrSpecial = /[\d\W]/.test(newPassword);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!hasMinLength || !hasNumOrSpecial) {
      setMsg({ type: 'error', text: 'Password must be at least 8 characters with at least 1 number or special character.' });
      return;
    }
  ...
  if (isSuccess) {
    return (
      <Box sx={{ display: 'flex', minHeight: '80vh', alignItems: 'center', justifyContent: 'center', px: 2 }}>
        <Paper elevation={4} sx={{ p: { xs: 2.5, sm: 4 }, maxWidth: 450, width: '100%', borderRadius: 3, textAlign: 'center', border: '1px solid #e2e8f0' }}>
          ...
          {msg && <Alert severity={msg.type} sx={{ borderRadius: 2 }}>{msg.text}</Alert>}
            ...
            <Stack spacing={3}>
              ...
              <Box sx={{ bgcolor: '#f8fafc', p: 2, borderRadius: 2, border: '1px solid #e2e8f0' }}>
                ...
                <Typography variant="caption" display="flex" alignItems="center" gap={0.5} color={hasMinLength ? 'success.main' : 'text.disabled'}>
                  {hasMinLength ? <CheckCircleIcon fontSize="inherit" /> : <span>&bull;</span>} At least 8 characters
                </Typography>
                <Typography variant="caption" display="flex" alignItems="center" gap={0.5} color={hasNumOrSpecial ? 'success.main' : 'text.disabled'}>
                  {hasNumOrSpecial ? <CheckCircleIcon fontSize="inherit" /> : <span>&bull;</span>} Contains a number or special character
                </Typography>
                <Typography variant="caption" display="flex" alignItems="center" gap={0.5} color={matchesConfirm ? 'success.main' : 'text.disabled'}>
                  {matchesConfirm ? <CheckCircleIcon fontSize="inherit" /> : <span>&bull;</span>} Passwords match correctly
                </Typography>
              </Box>
              ...
              <Button
                ...
                fullWidth
                variant="contained"
                disabled={!token || !email || loading || !hasMinLength || !hasNumOrSpecial || !matchesConfirm}
                sx={{ 
                  py: 1.5, 
                  fontWeight: 'bold', 
                  borderRadius: 2, 
